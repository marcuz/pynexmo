...
final_url = api_url + '/' + api_path + "username=" + api_user + "&password=" + api_pass + "&from=" + sms_from + "&to=" + sms_to + "&text=" + sms_msg

if confirm(sms_from, sms_to, sms_msg):
	res = fetch(url_fix(final_url))
	if res['messages'][0]['status'] == "0":
		print "SMS succesfully sent to '" + sms_to + "'. Available balance: " + res['messages'][0]['remaining-balance']
	else:
		print "Something went wrong, you suuuck (or maybe API do)! Error message: '" + res['messages'][0]['error-text'] + "'"
